...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  const rows = await ctx.env.DB.prepare(`
    SELECT md.*, p.name as assigned_participant_name, p.initials as assigned_participant_initials,
      (SELECT COUNT(*) FROM fixtures f WHERE f.kickoff_local_date = md.local_date) as fixture_count,
      (SELECT COUNT(*) FROM bets b WHERE b.match_day_id = md.id AND b.settlement_status = 'pending') as pending_bets_count,
